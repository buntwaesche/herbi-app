...
export async function GET() {
  ...
  // 1. Berufsfelder laden
  const { data: berufsfelder, error: bfError } = await supabase.from('berufsfelder').select('id, name');
  if (bfError) return NextResponse.json({ error: 'Berufsfelder laden fehlgeschlagen', details: bfError.message }, { status: 500 });

  const bfMap: Record<string, number> = {};
  berufsfelder!.forEach((bf: { id: number; name: string }) => { bfMap[bf.name] = bf.id; });
  ...
  await supabase.from('referenten').delete().gte('id', 0);
  results.push('Referenten gelöscht');

  // 4. Referenten einfügen
  let insertedCount = 0;
  let zuordnungCount = 0;
  const errors: string[] = [];

  for (const ref of referentenData) {
    const { berufsfeld, ...refFields } = ref;

    const insertData: Record<string, unknown> = {
      vorname: refFields.vorname,
      nachname: refFields.nachname,
      email: refFields.email,
      club: refFields.club,
      bemerkung: refFields.bemerkung || null,
      status: refFields.status,
      anzeigen: refFields.anzeigen,
    };
    ...
    if (error) {
      errors.push(`Referent ${refFields.vorname} ${refFields.nachname}: ${error.message}`);
      continue;
    }
    insertedCount++;
    ...
    if (berufsfeld && bfMap[berufsfeld]) {
      ...
      if (zError) {
        errors.push(`Zuordnung ${refFields.nachname} → ${berufsfeld}: ${zError.message}`);
      } else {
        zuordnungCount++;
      }
    } else if (berufsfeld) {
      errors.push(`Berufsfeld nicht gefunden: "${berufsfeld}" (für ${refFields.nachname})`);
    }
  }

  return NextResponse.json({
    success: true,
    referenten_importiert: insertedCount,
    zuordnungen_erstellt: zuordnungCount,
    hinweise: errors,
    log: results,
  });
}
